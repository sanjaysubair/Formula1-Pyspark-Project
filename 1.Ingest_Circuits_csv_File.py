...
circuits_df = spark.read.option("header", True).option("inferSchema", True).csv("abfss://raw@sanjayformula1dl2.dfs.core.windows.net/circuits.csv")
display(circuits_df)
...
circuits_df1 = spark.read.schema(Circuits_Schema).option("Header", True).csv("abfss://raw@sanjayformula1dl2.dfs.core.windows.net/circuits.csv")
...
circuits_final_df = circuits_renamed_df.withColumn("TimeStamp", current_timestamp())\
